...
//	    return timer;
	...
	public void resetTimer(Object timerObjs) {
		...
		Timer timer = (Timer) objs[0];
		Integer delay = (Integer) objs[3];

		timer.cancel();

		if (delay >= 0) {
			objs[0] = timer = new Timer();
...
//        stdoutElement.setScrollTop(Math.max(0, stdoutElement.getScrollHeight() - stdoutElement.getClientHeight()));
    ...
    private static HTMLButtonElement examplesButton = HTMLDocument.current().getElementById("select").cast();
	...
	public static void main(final String argv[]) {
        ...
        canvas = document.getElementById("canvas").cast();
        graphics = canvas.getContext("2d").cast();
        code = document.getElementById("code").cast();
	...
	//				this.mousepos = mousepos;
        ...
        document.getElementById("run").addEventListener("click", new EventListener() {
			@Override
			public void handleEvent(Event evt) {
				if (process != null)
				...
				try {
					process.run(argv, new ProcessReturnHandler() {
