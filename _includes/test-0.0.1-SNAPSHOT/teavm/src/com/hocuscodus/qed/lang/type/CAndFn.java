...
public class CAndFn extends BinaryFn {
	public CAndFn() {
	...
	public void writeExp(Exp exp, DecEnv env, Path path, OutputStream out) throws IOException {
		...
		if (binaryExp.value1.getReturnType(env) == Type.booleanType) {
			...
			new BinaryExp(Type.ifStackFn, binaryExp.value1, new ValueExp(Type.intType, body.length + 5)).write(env, path, out);
			out.write(body);
			bodyContent.reset();
			new ValueExp(Type.booleanType, false).write(env, path, bodyContent);
