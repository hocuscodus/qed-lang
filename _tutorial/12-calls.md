...
So classes can act like functions for calls but the internal process is different. What happens under the hood is, a temporary internal object is created and executed. Upon reaching the `return` call, the execution is stopped and the return value (if the class return type is not `void`) is returned to the caller, which resumes its own execution with it.
